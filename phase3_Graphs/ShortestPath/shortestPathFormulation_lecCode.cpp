// Look at BudgetTravelling.cpp - same thing!
...
int dist[1001][1001];   // stores the minimum distance to reach a node with the given amount of petrol left
// in dijkstra, we can't overwrite visited nodes with distance just like bfs
int vis[1001][1001];    // stores if a node is visited with the given amount of petrol left unused
...
// vis check could be avoided before pushing to pq check
// coz we are already checking if dist is better
