...
/** TODO:
 * Importance of visited array in dijkstra
 * why pq over queue
*/
    ...
    while(!pq.empty()){
        ...
        for(auto x: adj[curr]){
            if(cost[x.first] > cost[curr] + x.second){
                ...
                minFlight[x.first] = min(minFlight[x.first], minFlight[curr] + 1);
                maxFlight[x.first] = max(maxFlight[x.first], maxFlight[curr] + 1);
            }
        }
    }
