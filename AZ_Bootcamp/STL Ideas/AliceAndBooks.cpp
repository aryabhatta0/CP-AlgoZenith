...
// Firstly you'd see a monotonic space, but ah the check() will be O(n)
// Why? lower_bound on set isn't the same :(

/** Intuition:
#1
 * Observe that 'ans' can only inc by 1 at a time
 * Think if 'B' is the (prev) optimal ans, when can 'B+1' be the answer as well ?
    when ( cnt of books >= B+1 ) is >= B+1

 * You'll need to store all the books with pages >= 'B+1', if it's count is > 'B' , 'B+1' is the ans.
 * How will you store such books?
 
#2
 * Do we really need to store the books? Think of O(n) way.
 * What we need is just the cnt of books with > B pages ! How can we update cnt, when we update B to B+1?
 * Won't we need just the freq of books with pages B+1 ? 
 * You could just subtract freq[B+1]. So let's store freq only.
 */
